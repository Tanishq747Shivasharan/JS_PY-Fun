...
for ch in u_msg:
    if ch.isalpha():
        if ch.isupper():
            encrypted = chr(((ord(ch) - ord('A') + u_num) % 26) + ord('A')), end=''
        else:
            encrypted = chr(((ord(ch) - ord('a') + u_num) % 26) + ord('a')), end=''
    else:        
        print(ch, end='')
        print(f"Encrypted message: {encrypted}", end='')
...
for ch in encrypted
    if ch.isalpha():
        if ch.isupper():
            decrypted = chr(((ord(ch) - ord('A') - u_num) % 26) + ord('A')), end=''
        else:
            decrypted = chr(((ord(ch) - ord('a') - u_num) % 26) + ord('a')), end=''
    else:
        print(ch, end='')
        print(f"Decrypted message: {decrypted}", end='')